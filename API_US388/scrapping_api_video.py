...
def search_pixabay_video(query, api_key,processed_video_paths, per_page=3, safe_search=True):
    ...
    processed_video_paths=[path.split('.mp4')[0].replace("datalake_media/video-", "") + '.mp4' if '.mp4' in path else path.replace("datalake_media/video-", "") for path in processed_video_paths]
    url = f'https://pixabay.com/api/videos/?key={api_key}&q={query}&per_page={per_page}&safesearch={safe_search_param}'
    response = requests.get(url)
    videos_data = response.json()['hits']
...
def download_media(media):
    ...
    if filename == '':
        ...
        if video_opened==True :
            ...
            s3_key = f"datalake_media/video-{filename}-{str(uuid.uuid4())}"
            put_image_in_s3(requests.get(url).content, output_bucket, s3_key)
        else: 
            s3_key=None
